...
def red(x):
    if x in string.ascii_lowercase: return "a"
    if x in string.ascii_uppercase: return "A"
...
def scrape_links_and_wordlistify(links, lower=False):
    ...
    for site in links:
        try:
            ...
            raw_text = BeautifulSoup(raw, 'html.parser').get_text(separator=' ', strip=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-w', '--weights', help='Store the weights (occurance) for each entry', action='store_true')
